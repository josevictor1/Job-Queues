...
;(println (have-skill? "bills-questions" (:primary_skillset (:new_agent (first (get-agents datareceived))))))

;(println (filter #(have-skill? (:type (:new_job (first jobs))) (:primary_skillset (:new_agent  %))) agents))

(defn filter-primary [job agents] 
  (filter #(have-skill? (:type (:new_job  job)) (:primary_skillset (:new_agent  %))) agents))

(defn filter-secondary [job agents]
  (filter #(have-skill? (:type (:new_job  job)) (:secondary_skillset (:new_agent  %))) agents)) 
...
;(println (mountjob_assigned (first jobs) (first agents)))

(defn dequeue [coll_jobs coll_agts]
  (loop [jobs coll_jobs agts coll_agts result []]
    (if (or (empty? jobs) (empty? agts)) 
      result
      (let [primary (filter-primary (first jobs) agts) secondary (filter-secondary (first jobs) agts)] 
        (cond 
          (not (empty? primary)) 
